...
CITIES = [
    ...
    # Arkansas
    ("West Memphis", "AR", 35.1465, -90.1846), ("Cabot", "AR", 34.9537, -92.0165),
    ("Searcy", "AR", 35.2506, -91.7362), ("El Dorado", "AR", 33.2076, -92.6663),
    ("Mountain Home", "AR", 36.3356, -92.3854),
    # California
    ("Palmdale", "CA", 34.5794, -118.1165), ("Lancaster", "CA", 34.6868, -118.1542),
    ("Visalia", "CA", 36.3302, -119.2921), ("Vallejo", "CA", 38.1041, -122.2566),
    ("San Mateo", "CA", 37.5630, -122.3255), ("Berkeley", "CA", 37.8716, -122.2727),
    ("Santa Cruz", "CA", 36.9741, -122.0308), ("Napa", "CA", 38.2975, -122.2869),
    ("Chico", "CA", 39.7285, -121.8375), ("Concord", "CA", 37.9780, -122.0311),
    # Delaware
    ("Camden", "DE", 39.1137, -75.5419), ("Harrington", "DE", 38.9237, -75.5777),
    ("Laurel", "DE", 38.5565, -75.5713),
    # Florida
    ("Lakeland", "FL", 28.0395, -81.9498), ("Deltona", "FL", 28.9005, -81.2637),
    ("Melbourne", "FL", 28.0836, -80.6081), ("Kissimmee", "FL", 28.2920, -81.4076),
    ("Palm Coast", "FL", 29.5844, -81.2079), ("Sanford", "FL", 28.8029, -81.2732),
    # Maryland
    ("Bethesda", "MD", 38.9847, -77.0947), ("Silver Spring", "MD", 38.9907, -77.0261),
    ("Germantown", "MD", 39.1732, -77.2716),
    # New York
    ("New Rochelle", "NY", 40.9115, -73.7824), ("Mount Vernon", "NY", 40.9126, -73.8371),
    ("Niagara Falls", "NY", 43.0962, -79.0377), ("Jamestown", "NY", 42.0970, -79.2353),
    ("Glens Falls", "NY", 43.3095, -73.6440),
    # North Dakota
    ("Grafton", "ND", 48.4122, -97.4106), ("Bottineau", "ND", 48.8272, -100.4468),
    ("Rugby", "ND", 48.3689, -99.9962),
    # Rhode Island
    ("Johnston", "RI", 41.8220, -71.5148), ("North Providence", "RI", 41.8501, -71.4662),
    ("Coventry", "RI", 41.7001, -71.6828), ("Lincoln", "RI", 41.9209, -71.4351),
    # South Dakota
    ("Custer", "SD", 43.7667, -103.5988), ("Mobridge", "SD", 45.5372, -100.4279),
    ("Chamberlain", "SD", 43.8108, -99.3293), ("Winner", "SD", 43.3764, -99.8590),
    # Texas
    ("College Station", "TX", 30.6280, -96.3344), ("Temple", "TX", 31.0982, -97.3428),
    ("Killeen", "TX", 31.1171, -97.7278), ("Round Rock", "TX", 30.5083, -97.6789),
    ("Denton", "TX", 33.2148, -97.1331), ("San Marcos", "TX", 29.8833, -97.9414),
    ("Port Arthur", "TX", 29.8850, -93.9399), ("Texarkana", "TX", 33.4418, -94.0477),
    ("Nacogdoches", "TX", 31.6035, -94.6553), ("Victoria", "TX", 28.8053, -96.9853),
    # Illinois
    ("Belleville", "IL", 38.5201, -89.9840), ("Normal", "IL", 40.5142, -88.9906),
    ("DeKalb", "IL", 41.9295, -88.7503), ("Urbana", "IL", 40.1106, -88.2073),
    # Ohio
    ("Findlay", "OH", 41.0442, -83.6499), ("Ashtabula", "OH", 41.8650, -80.7898),
    ("Chillicothe", "OH", 39.3331, -82.9824),
    # Georgia
    ("Peachtree City", "GA", 33.3968, -84.5958), ("Gainesville", "GA", 34.2979, -83.8241),
    ("Statesboro", "GA", 32.4488, -81.7832),
    # North Carolina
    ("Boone", "NC", 36.2168, -81.6746), ("Sanford", "NC", 35.4799, -79.1803),
    ("Shelby", "NC", 35.2913, -81.5357),
    # Virginia
    ("Staunton", "VA", 38.1496, -79.0717), ("Radford", "VA", 37.1318, -80.5765),
...
def generate_zips_csv():
    ...
    for state, (zip_start, zip_end) in state_zip_ranges.items():
        ...
        # Generate ZIPs per state, spread around known city locations
        zip_range = zip_end - zip_start
        num_zips = min(250, max(80, zip_range // 12))
